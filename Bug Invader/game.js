...
function create() {
  ...
  this.physics.add.collider(pellets, gameState.player, () => {
    ...
    this.physics.pause();      
    this.add.text(120, 252, 'GAME OVER', { fontFamily: 'Chalkduster', fontSize: '35px', fill: '#3300FF' });
  });
  ...
  this.physics.add.collider(gameState.enemies, gameState.bugRepellent, (bug, repellent) => {
    ...
    //nOT SURE IF SHOULD BE here or IN UPDATE
...
function update() {
    ...
    if (numOfTotalEnemies() === 0){
      gameState.active = false;
      this.physics.pause();
      this.add.text(120, 252, 'You are a WINNER!', {fontSize: '15px', fill: '#000000'});
    } else {
      gameState.enemies.getChildren().forEach(bug => {
        bug.x += gameState.enemyVelocity;
      });
      
      gameState.leftMostBug = sortedEnemies()[0]; //given function
